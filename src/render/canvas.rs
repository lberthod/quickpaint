...
/// Transformation document → écran (origine + échelle). MVP : pas de pan/zoom
/// utilisateur, mais l'API est prête pour les ajouter (section 10).
#[derive(Clone, Copy)]
...
impl ViewTransform {
    pub fn doc_to_screen(&self, p: (f32, f32)) -> Pos2 {
        self.origin + Vec2::new(p.0 * self.scale, p.1 * self.scale)
    }
